...
router.get('/downloadAuditData', async (req, res) => {
  ...
  try {
    if (employeeNumber) {
      ...
      INNER JOIN department d ON ad.DepartmentId = d.dept_id
      INNER JOIN Employees e ON e.emp_no = ad.EmployeeNo 
      WHERE CAST(ad.ScheduledStartDate as date) >= '${fromDate}' AND CAST(ad.ScheduledEndDate as Date) <= '${toDate}' AND ad.EmployeeNo = ${employeeNumber} AND (ad.AuditStatus = 'Closed' OR ad.AuditStatus = 'Expired')`;
    }
    else {
      query = `SELECT ad.Id, ad.EmployeeNo,
      CASE 
      ...
      INNER JOIN department d ON ad.DepartmentId = d.dept_id
      INNER JOIN Employees e ON e.emp_no = ad.EmployeeNo 
      WHERE CAST(ad.ScheduledStartDate as date) >= '${fromDate}' AND CAST(ad.ScheduledEndDate as Date) <= '${toDate}' AND (ad.AuditStatus = 'Closed' OR ad.AuditStatus = 'Expired')`;
    }
